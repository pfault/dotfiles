...
;; Save place in files
(setq save-place-file (expand-file-name "saveplace" nexus-cache-dir))
(save-place-mode 1)
...
(setq backup-by-copying t
      ...
      `((".*" . ,(expand-file-name "backup" nexus-cache-dir))))

;; Auto-save files
(let ((auto-save-dir (expand-file-name "autosave/" nexus-cache-dir)))
...
(setq savehist-additional-variables
      ...
      ;; keep the home clean
      savehist-file (expand-file-name "savehist" nexus-cache-dir))
(savehist-mode +1)

;; Recentf
(require 'recentf)
(setq recentf-save-file (expand-file-name "recentf" nexus-cache-dir)
      recentf-max-saved-items 5000
      recentf-max-menu-items 1000
      ;; disable recentf-cleanup on Emacs start, because it can cause
      ;; problems with remote files
      recentf-auto-cleanup 'never
      recentf-exclude '("\\.git.*" "\\.hg.*" "\\.svn.*" "\\.ccls-cache.*"))

(defun nexus-recentf-exclude-p (file)
  "A predicate to decide whether to exclude FILE from recentf."
  (let ((file-dir (file-truename (file-name-directory file))))
    (-any-p (lambda (dir)
              (string-prefix-p dir file-dir))
            (mapcar 'file-truename (list nexus-cache-dir package-user-dir)))))

(add-to-list 'recentf-exclude 'nexus-recentf-exclude-p)
...
(defun yank-advised-indent-function (beg end)
  ...
  (if (<= (- end beg) nexus-yank-indent-threshold)
...
(advise-commands "indent" (yank yank-pop) after
  "If current mode is one of `nexus-yank-indent-modes',
indent yanked text (with prefix arg don't indent)."
  (if (and (not (ad-get-arg 0))
           (not (member major-mode nexus-indent-sensitive-modes))
           (or (derived-mode-p 'prog-mode)
               (member major-mode nexus-yank-indent-modes)))
